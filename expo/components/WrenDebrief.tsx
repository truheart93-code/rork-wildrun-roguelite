...
export default function WrenDebrief({ stats, skullsEarned, totalSkulls, victory, onContinue }: DebriefProps) {
  ...
  useEffect(() => {
    ...
    typingRef.current = setInterval(() => {
      ...
      if (i >= fullText.length) { clearInterval(typingRef.current!); setTyping(false); }
    }, 20);
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        {victory ? <Trophy size={24} color={COLORS.gold} /> : <Skull size={24} color={COLORS.red} />}
      ...
      <View style={styles.statsStrip}>
        {[
          { label: 'FLOORS', value: String(stats.floorsCleared), color: COLORS.white },
          { label: 'CAUGHT', value: String(stats.animalsCaught), color: COLORS.green },
          { label: 'CRITS', value: String(stats.criticalHits), color: COLORS.gold },
          { label: 'SKULLS', value: '+' + skullsEarned, color: COLORS.gold },
        ].map(s => (
          <View key={s.label} style={styles.statPill}>
            <RetroText variant="label" color={COLORS.grayDark} style={styles.statPillLabel}>{s.label}</RetroText>
            <RetroText variant="bodyBold" color={s.color} style={styles.statPillValue}>{s.value}</RetroText>
          </View>
        ))}
      </View>
      <View style={styles.sceneArea}>
        <Animated.View style={[styles.wrenArea, { transform: [{ translateY: wrenFloat }] }]}>
          <View style={styles.wrenBadge}>
            ...
            <RetroText variant="label" color={victory ? COLORS.green : COLORS.red} style={styles.wrenName}>DR. WREN</RetroText>
            <RetroText variant="body" color={COLORS.grayDark} style={styles.wrenRole}>Post-Run Analysis</RetroText>
          </View>
        ...
        <View style={styles.detailStats}>
          {[
            { label: 'DAMAGE DEALT', value: String(stats.totalDamageDealt), color: COLORS.red },
            { label: 'DAMAGE TAKEN', value: String(stats.totalDamageTaken), color: COLORS.whiteDim },
            { label: 'BEST HIT', value: stats.biggestHit + (stats.biggestHitAnimal ? ' (' + stats.biggestHitAnimal + ')' : ''), color: COLORS.gold },
            { label: 'WIN STREAK', value: String(stats.longestStreak), color: COLORS.green },
            { label: 'TURNS PLAYED', value: String(stats.turnsPlayed), color: COLORS.whiteDim },
            { label: 'TOTAL SKULLS', value: String(totalSkulls), color: COLORS.gold },
          ].map(s => (
            <View key={s.label} style={styles.detailRow}>
              <RetroText variant="label" color={COLORS.grayDark} style={styles.detailLabel}>{s.label}</RetroText>
              <RetroText variant="bodyBold" color={s.color} style={styles.detailValue}>{s.value}</RetroText>
            </View>
          ))}
        </View>
      </View>
      <Animated.View style={[styles.dialogueBox, { opacity: fadeAnim, borderTopColor: victory ? COLORS.green + '60' : COLORS.red + '40' }]}>
        <View style={styles.dialogueHeader}>
          ...
          <RetroText variant="label" color={victory ? COLORS.green : COLORS.red} style={styles.speakerName}>DR. WREN</RetroText>
          <RetroText variant="label" color={COLORS.grayDark} style={styles.pageCount}>{page + 1}/{lines.length}</RetroText>
        </View>
        <View style={styles.textArea}>
          <RetroText variant="body" color={COLORS.whiteDim} style={styles.dialogueText}>{displayedText}</RetroText>
        </View>
...
const styles = StyleSheet.create({
  ...
  statsStrip: { flexDirection: 'row', marginHorizontal: 12, marginBottom: 8, backgroundColor: COLORS.bgCard, borderRadius: 10, overflow: 'hidden', borderWidth: 1, borderColor: '#222826' },
  ...
  wrenBadge: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: COLORS.bgCard, borderRadius: 8, paddingHorizontal: 14, paddingVertical: 8, borderWidth: 1, borderColor: '#222826' },
  ...
  detailStats: { backgroundColor: COLORS.bgCard, borderRadius: 10, padding: 12, borderWidth: 1, borderColor: '#222826', gap: 8 },
  detailRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  detailLabel: { fontSize: 8 },
  detailValue: { fontSize: 14 },
  dialogueBox: { backgroundColor: COLORS.bgCard, borderTopWidth: 2, paddingHorizontal: 16, paddingTop: 14, paddingBottom: 12, gap: 10 },
