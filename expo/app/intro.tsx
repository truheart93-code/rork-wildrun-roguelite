...
export default function IntroScreen() {
  ...
  const goToGame = () => router.replace('/starter-camp');
  ...
  const handleNext = () => {
    ...
    if (page < DIALOGUE.length - 1) { setPage(p => p + 1); }
    else { goToGame(); }
  };
  ...
  const renderText = () => {
    if (!current.highlight || !displayedText.includes(current.highlight)) {
      return <RetroText variant="body" color={COLORS.whiteDim} style={styles.dialogueText}>{displayedText}</RetroText>;
    }
    const parts = displayedText.split(current.highlight);
    return (
      <RetroText variant="body" color={COLORS.whiteDim} style={styles.dialogueText}>
        {parts[0]}
        <RetroText variant="bodyBold" color={COLORS.green} style={styles.dialogueText}>{current.highlight}</RetroText>
        {parts[1] ?? ''}
      </RetroText>
    );
  };
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <TouchableOpacity style={styles.skipBtn} onPress={goToGame}>
        <RetroText variant="label" color={COLORS.grayDark} style={styles.skipText}>SKIP ›</RetroText>
      </TouchableOpacity>
      <View style={styles.sceneArea}>
        <Animated.View style={[styles.bgAnimal, { left: '5%', top: 20 }]}><AnimalSilhouette animalId="lion" color={COLORS.savanna} size={40} /></Animated.View>
        <Animated.View style={[styles.bgAnimal, { right: '5%', top: 30 }]}><AnimalSilhouette animalId="orca" color={COLORS.ocean} size={36} /></Animated.View>
        <Animated.View style={[styles.bgAnimal, { left: '15%', bottom: 20 }]}><AnimalSilhouette animalId="gorilla" color={COLORS.jungle} size={32} /></Animated.View>
        <Animated.View style={[styles.bgAnimal, { right: '12%', bottom: 30 }]}><AnimalSilhouette animalId="polar_bear" color={COLORS.arctic} size={38} /></Animated.View>
      ...
      <Animated.View style={[styles.dialogueBox, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
        <View style={styles.dialogueHeader}>
          ...
          <RetroText variant="label" color={COLORS.green} style={styles.speakerName}>{current.speaker}</RetroText>
          {typing && <View style={styles.typingDots}>{[0,1,2].map(i=><View key={i} style={[styles.dot,{opacity:0.4+i*0.2}]}/>)}</View>}
        </View>
        <View style={styles.dialogueTextArea}>{renderText()}</View>
        <View style={styles.progressRow}>
          {DIALOGUE.map((_,i)=><View key={i} style={[styles.progressPip,i===page&&styles.progressPipActive,i<page&&styles.progressPipDone]}/>)}
        </View>
...
const styles = StyleSheet.create({
  ...
  bgTop: { position: 'absolute', top: 0, left: 0, right: 0, height: '55%', backgroundColor: '#0a1a0e' },
  bgBottom: { position: 'absolute', bottom: 0, left: 0, right: 0, height: '45%', backgroundColor: COLORS.bg },
  skipBtn: { position: 'absolute', top: 16, right: 16, zIndex: 10, padding: 8 },
  skipText: { fontSize: 9 },
  sceneArea: { flex: 1, alignItems: 'center', justifyContent: 'center', position: 'relative' },
  bgAnimal: { position: 'absolute', opacity: 0.12 },
  wrenContainer: { alignItems: 'center', gap: 12 },
  wrenBadge: { alignItems: 'center', backgroundColor: COLORS.bgCard, borderRadius: 8, paddingHorizontal: 16, paddingVertical: 8, borderWidth: 1, borderColor: COLORS.green + '40' },
  wrenTitle: { fontSize: 11 },
  wrenSubtitle: { fontSize: 12, marginTop: 2 },
  wrenSilhouettes: { flexDirection: 'row', gap: 8, alignItems: 'flex-end' },
  dialogueBox: { backgroundColor: COLORS.bgCard, borderTopWidth: 2, borderTopColor: COLORS.green + '60', paddingHorizontal: 20, paddingTop: 16, paddingBottom: 12, gap: 12 },
  ...
  nextBtn: { backgroundColor: COLORS.green, paddingVertical: 14, borderRadius: 8, alignItems: 'center' },
